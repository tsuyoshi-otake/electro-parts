...
- For "cheapest X": Akizuki prices are per `quantityUnit` (some are per 10 pcs / per set) — compare unit prices, not raw `amountYen`. Switch Science `prices.minYen` is always per item. Sengoku's `priceTaxIncluded` is the smallest tier, whose `minimumQuantity` may be 10 or more — for bulk needs quote the matching tier from `priceTiers`, which makes Sengoku often the cheapest source at quantity. Aitendo names encode the multiple instead (`（50本入）`, `2個入`) and range products span variations — read the name before comparing prices.
