...
「取り込み合計」は履歴を作るために 365〜1,825 回の取り込みを連続で回した合計で、実運用では 1 run ぶん(p95 で 0.5〜0.8 秒)だけです。両店舗の収集設定は2日に1回・05:00 JSTです。この表は9,000商品の合成データであり、現在の全店舗の商品数・変化率での実測ではありません。5 年の p50 が 3 年より小さいのは同じマシンでの測定ばらつきです。「最大」は SQLite が WAL/ページを整理する run に当たったときの値。
...
- **収集間隔**: 両店舗とも約2日に1回の観測です。観測の間に起きた変化(短時間の値下げ、在庫の増減)は記録されません。「変化点の時刻」は**変化を初めて観測した時刻**で、実際に変わった時刻ではありません。契約v1.1の省略可能フィールド `observation.samplingIntervalDays: 2` から注意書きを表示します。旧版は未知のcaveatキーを拒否するため、この間隔を新しいキーとして配信しません。従来の月次・週次の注意書きも読み込めます。
