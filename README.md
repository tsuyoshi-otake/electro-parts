...
電子部品通販サイトの商品ページに、**観測した価格・在庫表示・掲載状況の履歴**を表示するプロジェクトです。

- クローラーが定期的に(現在は3店舗とも2日ごと → [ADR-0016](docs/adr/0016-per-store-observation-cadence.md))対象サイトのカタログを巡回して生スナップショットを保存し、
- 3店舗で共有する SQLite に**変化点だけ**を取り込み、
...
0.4.5 の対応表は3店舗間の943組です。同一商品は保存カタログ照合済み810組・未確定候補3組、類似商品は照合済み108組・候補6組、要確認は16組。秋月8,677商品（2026-09-06）、スイッチサイエンス10,343商品（2026-08-02）、M5Stack公式664商品・706バリエーション（2026-09-12）の全行を探索しました。型番一致候補830組を確認し、公式ショップとの同一商品517組を追加しています。現在の全商品・全別名を網羅したものでも、詳細ページの全件再確認でもありません。 詳細は[3店舗の照合範囲](docs/roadmap/three-store-mapping.md)を参照してください。

M5Stack公式は確認したバリエーションのUSD参考記録価格・リンクを表示します。選択中と異なる構成は混ぜません。差額・重ね合わせの承認は、引き続き ATOM Lite と Arduino UNO R4 Minima の **2 組だけ**です。他は参考記録価格として、店舗ごとの観測日時・条件を表示します。関連リンクは省略せず、他商品の履歴取得は 1 ページ最大 8 商品・同時 2 件に制限します。設計は [ADR-0017](docs/adr/0017-cross-store-comparisons.md)、根拠ファイル・件数・更新手順は [ADR-0018](docs/adr/0018-reviewed-catalog-mapping-pipeline.md) を参照してください。

0.4.6では単店グラフも表示欄の幅に合わせて描画します。狭い画面で日付・価格の文字が小さくなりすぎる問題を修正し、画面幅を変えた場合も追従します。
...
Chrome ウェブストアに出すための準備は済んでいます。提出する文言(掲載情報・単一用途・権限の理由・データ使用の回答)と手順は [docs/chrome-web-store-listing.md](docs/chrome-web-store-listing.md) が原本です。掲載されるまでは、下の zip をデベロッパーモードで読み込んでください。
...
- **zip で入れた場合、自動更新はありません。** 版を上げたときは取り直してください(ユーザースクリプトは `@updateURL` で自動更新されます)。デベロッパーモードの拡張機能に関する Chrome の警告も毎回出ます。ストアに掲載されたあとは、ストア経由で自動更新されます。
...
- プライバシーポリシー: <https://tsuyoshi-otake.github.io/electro-parts/privacy.html>(原本は `public/privacy.html`)。
- 自分でビルドする場合:
...
`dist/extension/` が「パッケージ化されていない拡張機能」として読み込めるフォルダです。`--zip <ファイル>` を付けると配布用 zip も作ります。マニフェストとアイコンは生成物で、手書きしません。掲載用のスクリーンショット(1280×800)も生成物です:

```bash
npm run build:store-assets
```

E2E と同じ保存済み商品ページ・同じ未展開拡張を実ブラウザに読み込んで撮るので、パネルを変えれば画像も変わります(`npx playwright install chromium` が必要)。4 枚のうち先頭 2 枚は店舗をまたいだ価格比較で、公開済みデータセットを読みます(比較には 1 商品について両店舗の記録が必要で、リポジトリ内のスナップショットには対応表のある組が入っていないため)。残り 2 枚はスナップショットから作ったデータセットで撮ります。同じコマンドがプロモーション タイル(440×280 と 1400×560)も作ります。ショップアイコンは生成せず、パッケージ同梱の `dist/extension/icons/icon-128.png` をそのまま使います。
審査で機械的に落ちる条件 — 名前 45 文字・説明 132 文字の上限、版番号の形式、アップロードで禁止されるマニフェストキー、余分な権限、`eval` などのリモートコード、コンテンツスクリプトからの通信 — は `tests/contract/chrome-web-store.test.ts` が検査します([ADR-0020](docs/adr/0020-chrome-web-store-submission.md))。
...
| 契約 | `tests/contract/` | 生成物の検証、決定性(同じ DB → 同じ `datasetVersion`)、Chrome 拡張マニフェストが Page Adapter 登録簿・データホスト・版と一致すること、ウェブストアの審査条件(文字数・版形式・禁止キー・権限・リモートコード不在・プライバシーポリシーの公開)を満たすこと |
...
| E2E(Playwright) | `tests/e2e/` | ビルド済みユーザースクリプトを**保存済み**商品ページ(秋月とスイッチサイエンス)で実行。店舗ページとデータの両オリジンを route interception で提供し、本物のサイトには触れません。キャッシュ再利用、障害時の fail-open、未収録商品、コレクション URL 経由の同一性、対応表のない商品では自店だけを読むこと、店舗横断比較の双方向性・取得上限・狭幅表示を検証。**展開済みの Chrome 拡張を実際に読み込む** spec もあり、サービスワーカーが取得したデータでパネルが出ること・通信が 2 回であることを確認します。M5Stackの保存済みページでは、USD価格・バリエーション切替・テーマ保存・狭幅表示を拡張機能で検証します |
...
実績: ユニット〜統合 433 テスト(39 ファイル、13 s)、E2E 11 テスト(11 s)、変異スコア **89.60 %**(704 変異体: killed 595 / timeout 8 / survived 60 / no coverage 10 / ignored 31、しきい値 break 70)。
...
| ユーザースクリプト | 1 商品ページで通信 2 回以下(マニフェスト + 商品)、キャッシュ有効時 0 回。描画コード 100 KB 以下(同梱する生成済み対応表は別勘定) |
...
「取り込み合計」は履歴を作るために 365〜1,825 回の取り込みを連続で回した合計で、実運用では 1 run ぶん(p95 で 0.5〜0.8 秒)だけです。3店舗の収集設定は2日に1回・05:00 JSTです。この表は9,000商品の合成データであり、現在の全店舗の商品数・変化率での実測ではありません。5 年の p50 が 3 年より小さいのは同じマシンでの測定ばらつきです。「最大」は SQLite が WAL/ページを整理する run に当たったときの値。
...
観測回数は 4.33 倍でも配信サイズは 1.23 倍にしかなりません。変化点中心で保存し、商品名や型番のような 1 商品あたりの固定費が観測回数で増えないからです。隔日化後もアクセス間隔を維持します。長期容量の制約と検証方法は[収集頻度と表示の検証](docs/observation-verification.md)を参照してください。

実データ(統合テストの保存済みスナップショット): 秋月は 2 run(2026-08-02、2026-09-06)から 8,809 商品ファイルを生成。これは 18 ジャンルを手書きしていた頃のスナップショットで、サイトマップ由来の巡回(ADR-0012)に切り替えたあとの公開データは 12,772 商品(datasetVersion `3ccce668810808ca`、2026-09-06〜09-07 の 2 run)。スイッチサイエンスは 2026-09-07 の実クロール(10,382 商品、カタログ 42 ページ、商品サイトマップ 11)から 60 商品を切り出したものを使い、`¥165` のカメラケーブルから `¥8,910,000` の装置まで、価格 0 円の 4 商品も含めて写像を検証しています。0.4.1のユーザースクリプトは対応表を含め677,674バイト(圧縮なし)。内訳は生成済み対応表が588,237バイト、描画コードが89,437バイトで、上の予算表の「描画コード 100 KB 以下」はこの後者を指します。Chrome 拡張は同じバンドルで、配布 zip が 90,619 バイト、展開後が 683,672 バイト(バンドル + マニフェスト + 生成アイコン 4 枚)。照合元カタログ全体はバンドルに含めません。
...
- **収集間隔**: 3店舗とも約2日に1回の観測です。観測の間に起きた変化(短時間の値下げ、在庫の増減)は記録されません。「変化点の時刻」は**変化を初めて観測した時刻**で、実際に変わった時刻ではありません。契約v1.1の省略可能フィールド `observation.samplingIntervalDays: 2` から注意書きを表示します。旧版は未知のcaveatキーを拒否するため、この間隔を新しいキーとして配信しません。従来の月次・週次の注意書きも読み込めます。
...
- Chrome 拡張も同じで、Cookie もテレメトリもありません。公開しているプライバシーポリシーは `public/privacy.html`(→ `/privacy.html`)で、GitHub Pages 側に通常のアクセスログが残り得ることも含めて書いています。取得はサービスワーカーが `credentials: 'omit'` で行い(店舗のページは発行元にならない)、受け付けるメッセージは 1 種類・送り主は自分の拡張のみ・URL はデータオリジンのみ・待ち時間の上限は 30 秒です。権限は `storage` とそのオリジンだけで、キャッシュは `chrome.storage.local`。
...
| [0020](docs/adr/0020-chrome-web-store-submission.md) | 審査に通る形をテストで固定し、Chrome ウェブストアに出す |
