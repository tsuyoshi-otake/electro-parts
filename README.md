...
E2E と同じ保存済み商品ページ・同じ未展開拡張を実ブラウザに読み込んで撮るので、パネルを変えれば画像も変わります(`npx playwright install chromium` が必要)。4 枚のうち先頭 2 枚は店舗をまたいだ価格比較で、公開済みデータセットを読みます(比較には 1 商品について両店舗の記録が必要で、リポジトリ内のスナップショットには対応表のある組が入っていないため)。残り 2 枚はスナップショットから作ったデータセットで撮ります。
