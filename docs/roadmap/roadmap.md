...
秋月電子通商・スイッチサイエンス・M5Stack公式ショップが**実装済み**、aitendo が**未実装**です。M5StackのUSD・全商品収集・バリエーション対応は [対応仕様](m5stack-support.md) を参照してください。エレショップは収集規模と一括提供元が課題のため保留です。
...
- 為替換算と異なる通貨間の価格比較（USDそのものの記録・表示はv1で対応済み）。
