 ...
 * state. The raw text is always preserved in `rawStatus`.
 */
export function mapAkizukiAvailabilityState(status: string | null, purchasable: boolean): AvailabilityState {
  const s = (status ?? '').trim();
...
export function normalizeAkizukiAvailability(stock: AkizukiRawStock): Availability {
  const purchasable = stock.purchasable === true;
  const quantity =
