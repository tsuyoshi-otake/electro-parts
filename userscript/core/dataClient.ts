...
export class DataClient {
  ...
  async load(storeId: string, pageKey: string, emit: (state: LoadState) => void): Promise<void> {
    ...
    const usable = cached !== null && now - cached.storedAt <= this.productMaxAgeMs ? cached : null;
  ...
  private async loadManifest(storeId: string, now: number): Promise<ManifestV1> {
    const cached = await this.cache.getManifest(storeId).catch(() => null);
    if (cached !== null && now - cached.storedAt <= this.manifestTtlMs && validateManifestV1(cached.body).length === 0) return cached.body as ManifestV1;
